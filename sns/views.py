...
from .forms import GroupCheckForm,GroupSelectForm,SearchForm,FriendForm,CreateGroupForm,PostForm
...
def groups(request):
  ...
  if request.method == 'POST':
    ...
    # Groupsメニュー選択肢の処理
    if request.POST['mode'] == '__groups_from__':
      # 選択したGroups名を取得
    ...
    if request.POST['mode'] == '__friends_form__':
      ...
      # フォームの用意
      groupform = GroupSelectForm(request.user, {'groups':sel_group})
      friendsform = FriendsForm(request.user, friend=friends, vals=vlist)
  
  # GET アクセス時の処理
  else:
    # フォームの用意
    groupsform = GroupSelectForm(request.user)
    friendsform = FriendsForm(request.user, friend=friends, vals=[])
    sel_group = '-'
...
def add(request):
  ...
  frd.save()
  # メッセージを設定
  messages.success(request, add_user.username + ' を追加しました！ groupページ移動して、追加したFriendをメンバーに設定してください。')
  return redirect(to='/sns')
...
def share(request, share_id):
  ...
  if request.method == 'POST':
    ...
    content = request.POST['content']
    # Group の取得
    group = Group.objects.filer(owner=request.user).filter(title=gr_name).first()
  if group == None:
    (pub_user, group) = get_public()
  # メッセージを作成し、設定をして保存
  msg = Message()
  ...
  msg.save()
  # メッセージを設定
  message.success(request, 'メッセージをシェアしました！')
  return redirect(to='/sns')
...
def get_your_group_message(owner, glist, find):
  ...
  if find==None:
    message = Message.objects.filter(Q(group__in=groups)|Q(group__in=me_groups))[:100]
  else:
    message = Message.objects.filter(Q(group__in=groups)|Q(group__in=me_groups)).filter(content__contains=find)[:100]
  return messages
